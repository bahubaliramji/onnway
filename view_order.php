<?php   session_start();
 ...
 include("TBXadmin/include/config.php"); 
   ...
   $data = mysqli_fetch_array($resultB);
	...
	$seo_keyword = "Order Information";
	$seo_content = "Order Information";
	...
	?>
       ...
       <div class="col-md-8">
          ...
          <table class="table table-bordered table-responsive">
              ...
              <tbody>
                <tr> <?php $sql_result = mysqli_fetch_array(mysqli_query($dbhandle, "select * from tbl_loader where id='".$_SESSION['user_id']."'"));?>
                  <td> <?php echo $sql_result['company_name'];?> </td>
                  <td> <?php echo $sql_result['landline_no'];?> </td>
                  <td><?php echo $sql_result['address'];?><br> <?php echo getCity($sql_result['city']);?> -<?php echo $sql_result['pincode'];?></td>
                </tr>
              </tbody>
            </table>
          </div>
