...
$row = mysqli_query($dbhandle, "select * from tbl_post_truck order by id desc");
  mysqli_query($dbhandle, "update tbl_post_truck set news=1 ");
$x=1;
while($fetch = mysqli_fetch_array($row)){

  $vehicledata = mysqli_fetch_assoc(mysqli_query($dbhandle, "select * from vehicle_list where id='".$fetch['truck_type']."'"));
 ...
 $vehicl = mysqli_fetch_assoc(mysqli_query($dbhandle, "select * from tbl_vehicle_owner where vehicle_owner_id='".$fetch['vehicle_owner_id']."'"));

													echo strtoupper($vehicl['vehicle_owner_type']); 
													?></td>

													<td><?php if($fetch['status']==1){ echo 'Pending';} else{
													echo 'Assigned';	
													} 
													...
													<td>
                                                     <?php echo $vehicledata['dimension'].' / '.$vehicledata['length'];?>MT
														<!-- <a href="truck_details.php?id=<?php echo $fetch['truck_id'];?>"><?php echo getRegNoBytruckId($fetch['truck_id']);?></a> -->
													...
													// echo  getVehicleOwnerInfo($fetch['vehicle_owner_id'])['email'];
                                           
											  echo getCity($fetch['source'])."-";
										
											  $destinations = explode(',',$fetch['destination']);
 
											  
												$cm = '';
												foreach($destinations as $id =>$value){
													echo $cm.getCity($value);
													$cm = ',';
												}
