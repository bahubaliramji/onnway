...
<?php
  ...
  $user_id=$_REQUEST['user_id'];  
         /*-----start fetch lat long--------*/
  
           $qrr=mysql_query("SELECT * FROM user_signup where id=$user_id");
            $rows=mysql_fetch_array($qrr);
              
             $latitude=$rows['latitude'];   $longitude=$rows['longitude']; 
  
         /*-----end fetch lat long--------*/

    //get fcm id
	 $qq="SELECT * FROM driver ORDER BY ((latitude-".$latitude.")*(latitude-".$latitude.")) + ((longitude - ".$longitude.")*(longitude - ".$longitude.")) ASC limit 1";
           $sel_fcm_id=mysql_query($qq);
		   
             while($row_fcm_id=mysql_fetch_array($sel_fcm_id))
             {
             			 
              $fcm_id=$row_fcm_id['fcmid']; 
