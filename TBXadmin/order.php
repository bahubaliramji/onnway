...
if (isset($_GET['del_id']))
{
	...
	if ($sql)
	{
		header("refresh:1;url=order.php");
											...
											<tbody>									<?php
$row = mysqli_query($dbhandle, "select * from tbl_book_load where status!=1  and lr_no!=''  order by lr_no desc");
$x = 1;
...
while ($fetch = mysqli_fetch_array($row))
{
	...
	<td width="141px; paging:2px;">							
		<a href="order_management_edit.php?edit_id=<?php
	echo $fetch['id']; ?>"><span class="btn btn-success btn-xs" id="edit21"><i class="fa fa-edit"></i>Edit</span></a>



<a href="track_order.php?id=<?php
	echo $fetch['id']; ?>"><span class="btn btn-success btn-xs" id=""><i class="fa fa-edit"></i>Track</span></a>


													<a style="margin-left: 10px;" href="order.php?del_id=<?php
