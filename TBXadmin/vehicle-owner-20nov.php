...
<?php include('include/sidebar.php'); ?>
	...
	$sql = mysql_query("DELETE FROM tbl_vehicle_owner WHERE vehicle_owner_id = '$deleteid'");
...
if($_GET['tag'])
{
	...
	$sid = $_GET['pids'];
	$query = mysql_query("UPDATE tbl_vehicle_owner SET status = '$prostatus' WHERE vehicle_owner_id = '$sid'");
	header("refresh:1;url=vehicle-owner.php");
...
$row = mysql_query("select * from tbl_vehicle_owner where 	vehicle_owner_type='owner' order by vehicle_owner_id desc");
$x=1;
...
while($fetch = mysql_fetch_array($row)){
	?>
											<tr>
													...
													<td><?php echo getCity($fetch['city']);?></td>
