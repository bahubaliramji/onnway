...
if (isset($_GET['del_id']))
{
	...
	if ($sql)
	{
		header("refresh:1;url=order.php");
											...
											<tbody>									<?php
$row = mysql_query("select * from tbl_book_load where status!=1 order by id desc");
$x = 1;

while ($fetch = mysql_fetch_array($row))
{
	$TruckLoadId = mysql_fetch_array(mysql_query("SELECT id,post_truck_id FROM tbl_post_truck WHERE id = '" . $fetch['assign_truck'] . "'"));
	 ?>										<tr>	
	<td><?php echo $x;?></td>			
	...
	<td>							<a href="order_management_edit.php?edit_id=<?php
