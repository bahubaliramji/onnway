...
if (isset($_GET['del_id']))
{
	...
	if ($sql)
	{
		header("refresh:1;url=order.php");
											...
											<tbody>									<?php
$row = mysqli_query($dbhandle, "select * from tbl_book_load where status!=1  and loading_ref_no!=''  order by loading_ref_no desc");
$x = 1;
...
while ($fetch = mysqli_fetch_array($row))
{
	$TruckLoadId = mysqli_fetch_array(mysqli_query($dbhandle, "SELECT id,post_truck_id FROM tbl_post_truck WHERE id = '" . $fetch['assign_truck'] . "'"));
	 ?>										<tr>	
	<td><?php echo $x;?></td>			
	<td><?php	echo $fetch['loading_ref_no']; ?></td>	
	...
	<td><?php	echo getorderstatus($fetch['order_status']); ?></td>		
	<td><?php	echo getorderstatus($fetch['advance_payment']); ?></td>		
	...
	echo $fetch['id']; ?>"><span class="btn btn-success btn-xs" id="edit21"><i class="fa fa-edit"></i>Edit</span></a>

	<a href="track_order.php?id=<?php
	echo $fetch['id']; ?>"><span class="btn btn-success btn-xs" id=""><i class="fa fa-edit"></i>Track</span></a>												<a style="margin-left: 10px;" href="order.php?del_id=<?php
