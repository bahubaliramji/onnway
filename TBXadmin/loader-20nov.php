...
<?php if(isset($_GET['del_id'])){
	...
	$sql = mysql_query("DELETE FROM tbl_loader WHERE id = '$deleteid'");
...
if($_GET['tag'])
{
	...
	$sid = $_GET['pids'];
	$query = mysql_query("UPDATE tbl_loader SET status = '$prostatus' WHERE id = '$sid'");
	header("refresh:1;url=loader.php?type='".$ldType."'");
...
$row = mysql_query("select * from tbl_loader where loader_type = '".$ldType."'");
$x=1;
