...
$path = $base_url."../upload/vehicle_documents/";
 ...
 echo '<script>window.location="vehicle_transporter_edit_reg.php?edit_id='.$_GET['edit_id'].'"</script>';
}
...
$other_docssql = ", other_docs = '$other_docs'";
		...
		$update = mysqli_query($dbhandle, "UPDATE tbl_vehicle_owner SET name = '$name', last_name = '$last_name', mb_no = '$mb_no', transport_name = '$transport_name', password='$password', address='$address', city='$city', pincode='$pincode', alt_contact_person='$alt_contact_person', alt_mb_no='$alt_mb_no', bank_name='$bank_name', branch_address='$branch_address', ifsc_code='$ifsc_code', benificiary_name='$benificiary_name',acc_no='$acc_no',company_name='$company_name',company_type='$company_type',gst_no = '$gst_no',pan_no='$pan_no',aadhar_no='$aadhar_no'  $pan_file $gst_file  $aadhar_cardsql $visiting_cardsql  $cancel_chequesql $other_docssql WHERE vehicle_owner_id = '".$_GET['edit_id']."'");

 if($update){
		$sms = '<p class="success-msg">Details Updated Successfully</p>' ;

 }else{
 			$sms = '<p class="error-msg">Something went wrong</p>' ;
