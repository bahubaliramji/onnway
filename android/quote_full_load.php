...
$ypath1 = "Uploads/material/" . $ythumb;
move_uploaded_file($ytmp_thumb, $ypath1);
...
$insquery = "INSERT INTO `orders`
 ...
 `drop_phone` , `remarks` , `length` , `width` , `height` , `quantity` , `status`,`material_image`,`sourceLAT`,`sourceLNG`,`destinationLAT`,`destinationLNG`) VALUES 
 ('$user_id', '$laod_type', '$source', '$destination', '$truck_type', '$schedule', '$weight', 
 '$material', '$paid_percent', '$paid_amount', 
 '$pickup_address', '$pickup_city', '$pickup_pincode', '$pickup_phone', '$drop_address', '$drop_city', '$drop_pincode', 
 '$drop_phone', '$remarks', '$length', '$width', '$height', '$quantity', 'requsted for quote','$imm','$sourceLAT','$sourceLNG','$destinationLAT','$destinationLNG')";
...
if ($row) {
  ...
  if ($laod_type == '1') {
    $row33 = mysqli_query($dbhandle, "INSERT INTO count SET full_load = 'read'");
  }
  if ($laod_type == '2') {
    $row33 = mysqli_query($dbhandle, "INSERT INTO count SET part_load = 'read'");
  }
  ...
  sendemail($iidd, $email, $name, $source, $destination, $try, $mat, $schedule, $lty);
