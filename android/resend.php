...
$phone = $_POST['phone'];
 ...
 //echo "SELECT * FROM users WHERE phone = '$phone'";
	$query = mysqli_query($dbhandle , "SELECT * FROM users WHERE phone = '$phone'");
	...
	if($count > 0)
	{
		$otp = rand(1000,9999);
		$sender = 'ONNWAY';
		$message = "<#> Your Otp for Registration on onnway.com is ".$otp." zQhGX8tdPOk";
		...
		$qqq = mysqli_query($dbhandle , "UPDATE users SET otp = '$otp' WHERE id = '$iidd'");
