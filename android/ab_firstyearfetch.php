<?php 	
	...
	$query = mysqli_query($dbhandle,"SELECT * FROM `ab_firstyear` where `route_name`='$route_name'");
		$response["devices"] = array();
	while($fetch = mysqli_fetch_array($query)){
	    if($fetch['changed_status']=='0'){
	          if($fetch['route_type']==$route_type){
	    $device['bus_no']=$fetch['bus_no'];
	...
	$query1 = mysqli_query($dbhandle,"SELECT * FROM `ab_firstyear` where `changed_route_name`='$route_name'");
		
	while($fetch1 = mysqli_fetch_array($query1)){
	    if($fetch1['changed_status']=='0'){
	          if($fetch['route_type']==$route_type){
	    $device['bus_no']=$fetch1['bus_no'];
