...
if ($_POST['id'] != "" && is_numeric($_POST['id'])){
     ...
     if(mysql_num_rows($query) > 0) {
			$query1 = mysql_query("SELECT * FROM tbl_post_truck WHERE truck_id = '".$_POST['id']."' order by id desc");
		if(mysql_num_rows($query1)>0){
			
			$response["is_error"] = false;
			$response["message"] = "All Booking List.";
			
			 $resultArray = array("all_booking"=>array());
			...
			array_push($resultArray['all_booking'],$comp_details);
