  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Unsubscribed Users</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
              <li class="breadcrumb-item active">Unsubscribed Users</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example3" class="table table-bordered table-striped">
				  ...
				  $query = "SELECT * FROM users WHERE (status = 'inactive' || status = 'unsubscribed') ORDER BY created DESC";
				  $run_query = mysqli_query($con , $query);
				  $sno = 0;
				  while($bdata = mysqli_fetch_array($run_query))
				  {
					  $sno++;
					  
	$profile_id = $bdata['id'];
	$status = $bdata['status'];
	$created = $bdata['created'];
	...
	else
	{
		...
		$sector = $sdata['sector'];
				  ...
				  <td><?= $type; ?></td>
					...
					{
					?>
					<td><a href="worker_profile.php?id=<?= $profile_id; ?>"><?= $name; ?></a></td>
					<?php
					}
					else if($type == 'brand')
					{
					?>
					<td><a href="brand_profile.php?id=<?= $profile_id; ?>"><?= $name; ?></a></td>
					<?php
					}
					else
					{
					?>
					<td><a href="contractor_profile.php?id=<?= $profile_id; ?>"><?= $name; ?></a></td>
					<?php
					}
					?>
                    <td><?= $phone; ?></td>
