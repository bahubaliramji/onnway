  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Ongoing Surveys</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="dashboard3.php">Home</a></li>
              <li class="breadcrumb-item active">Ongoing Surveys</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
				  ...
				  if($state == 'All States')
{
	...
	$query = "SELECT * FROM assigned_surveys WHERE (status = 'pending' || status = 'modifications') ORDER BY created DESC";
	//$qqq = mysql_query("SELECT * FROM assigned_surveys WHERE (status = 'pending' || status = 'modifications') ORDER BY created DESC");
}
else
{
	$query = "SELECT * FROM assigned_surveys WHERE state = '$state' AND (status = 'pending' || status = 'modifications') ORDER BY created DESC";
	...
	else
	{
		...
		$sector = $sdata['sector'];
				  ...
				  <td><?= $type; ?></td>
					...
					{
					?>
					<td><a href="pending_worker_profile2.php?id=<?= $profile_id; ?>&sid=<?= $row['id']; ?>"><?= $name; ?></a></td>
					<?php
					}
					else if($type == 'brand')
					{
					?>
					<td><a href="pending_brand_profile2.php?id=<?= $profile_id; ?>&sid=<?= $row['id']; ?>"><?= $name; ?></a></td>
					<?php
					}
					else
					{
					?>
					<td><a href="pending_contractor_profile2.php?id=<?= $profile_id; ?>&sid=<?= $row['id']; ?>"><?= $name; ?></a></td>
					<?php
					}
					?>
                    <td><?= $phone; ?></td>
