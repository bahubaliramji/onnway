...
<?php if(isset($_GET['del_id'])){
	...
	$sql = mysqli_query($dbhandle, "DELETE FROM tbl_post_truck_info WHERE id = '$deleteid'");
...
if($_GET['tag'])
{
	...
	$sid = $_GET['pids'];
	$query = mysqli_query($dbhandle, "UPDATE tbl_post_truck_info SET status = '$prostatus' WHERE id = '$sid'");
	header("refresh:1;url=booktruck.php");
						...
						<div class="col-xs-12">
							...
							<div class="box">
								<div class="box-header with-border">
										...
										<i class="fa fa-eye" aria-hidden="true"></i><h3 class="box-title">View Post a Truck List by Agent/Transporter</h3>
...
<div class="box-body box box-warning">		
                        ...
                        <ol class="breadcrumb">
                           <li><a href="dashboard.php"><i class="fa fa-dashboard"></i> Home</a></li>
							...
							<li class="active" style="color:#f0193f">Post Truck by Agent/Transporter</li>
						 </ol>
                      </div>
...
$row = mysqli_query($dbhandle, "select * from tbl_post_truck_info order by id desc");
$x=1;
				...
				<td><a href="vehicle_owner_details.php?id=<?php echo $fetch['vehicle_owner_id'];?>"><?php echo  getVehicleOwnerInfo($fetch['vehicle_owner_id'])['name'];?></a></td>
					...
					<!--<td> <?php if($fetch['status']=='active'){ ?><a href="booktruck.php?pids=<?=$fetch['id'];?>&tag=<?=$fetch['status'];?>" onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service"><span class="label label-success">active</span></a>
                     ...
                     <a href="booktruck.php?pids=<?=$fetch['id'];?>&tag=<?=$fetch['status'];?>" onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Service"><span class="label label-danger">Deactivate</span></a>
						...
						<td>
						<a href="book_truck_details_info.php?id=<?php echo $fetch['id'];?>"><span class="btn btn-success btn-xs"><i class="fa fa-edit"></i>View More</span></a>
                           <a href="book_truck_info_edit.php?edit_id=<?php echo $fetch['id'];?>"><span class="btn btn-success btn-xs"><i class="fa fa-edit"></i>Edit</span></a>
